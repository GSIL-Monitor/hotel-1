...
public class TestSupplierBillController extends BaseController {
...
//            list.add(billOrderDTO1);
     ...
     * 上传订单明细
     */
    @RequestMapping(value = "/uploadSupplierBillOrder", method = RequestMethod.POST, produces = { "application/json;charset=UTF-8" })
    @ResponseBody
    public ResponseDTO uploadSupplierBillOrder(@RequestBody UploadBillOrderDTO requestDTO) {
        ResponseDTO responseDTO = new ResponseDTO(ResultCodeEnum.SUCCESS.code);
        return responseDTO;
    }
     ...
     * 自动对账结果查询
     */
    @RequestMapping(value = "/querySupplyBillImport", method = RequestMethod.POST, produces = { "application/json;charset=UTF-8" })
    @ResponseBody
