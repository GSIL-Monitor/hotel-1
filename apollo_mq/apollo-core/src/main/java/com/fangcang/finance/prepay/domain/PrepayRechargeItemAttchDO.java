package com.fangcang.finance.prepay.domain;

import java.util.Date;
import javax.persistence.*;

@Table(name = "t_fin_prepay_recharge_item_attch")
public class PrepayRechargeItemAttchDO {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Integer id;

    /**
     * 充值明细id
     */
    @Column(name = "recharge_item_id")
    private Integer rechargeItemId;

    /**
     * 附件名称
     */
    private String name;

    /**
     * url地址
     */
    private String url;

    /**
     * 实际地址
     */
    private String realpath;

    private String creator;

    @Column(name = "create_time")
    private Date createTime;

    private String modifier;

    @Column(name = "modify_time")
    private Date modifyTime;

    /**
     * @return id
     */
    public Integer getId() {
        return id;
    }

    /**
     * @param id
     */
    public void setId(Integer id) {
        this.id = id;
    }

    /**
     * 获取充值明细id
     *
     * @return recharge_item_id - 充值明细id
     */
    public Integer getRechargeItemId() {
        return rechargeItemId;
    }

    /**
     * 设置充值明细id
     *
     * @param rechargeItemId 充值明细id
     */
    public void setRechargeItemId(Integer rechargeItemId) {
        this.rechargeItemId = rechargeItemId;
    }

    /**
     * 获取附件名称
     *
     * @return name - 附件名称
     */
    public String getName() {
        return name;
    }

    /**
     * 设置附件名称
     *
     * @param name 附件名称
     */
    public void setName(String name) {
        this.name = name;
    }

    /**
     * 获取url地址
     *
     * @return url - url地址
     */
    public String getUrl() {
        return url;
    }

    /**
     * 设置url地址
     *
     * @param url url地址
     */
    public void setUrl(String url) {
        this.url = url;
    }

    /**
     * 获取实际地址
     *
     * @return realpath - 实际地址
     */
    public String getRealpath() {
        return realpath;
    }

    /**
     * 设置实际地址
     *
     * @param realpath 实际地址
     */
    public void setRealpath(String realpath) {
        this.realpath = realpath;
    }

    /**
     * @return creator
     */
    public String getCreator() {
        return creator;
    }

    /**
     * @param creator
     */
    public void setCreator(String creator) {
        this.creator = creator;
    }

    /**
     * @return create_time
     */
    public Date getCreateTime() {
        return createTime;
    }

    /**
     * @param createTime
     */
    public void setCreateTime(Date createTime) {
        this.createTime = createTime;
    }

    /**
     * @return modifier
     */
    public String getModifier() {
        return modifier;
    }

    /**
     * @param modifier
     */
    public void setModifier(String modifier) {
        this.modifier = modifier;
    }

    /**
     * @return modify_time
     */
    public Date getModifyTime() {
        return modifyTime;
    }

    /**
     * @param modifyTime
     */
    public void setModifyTime(Date modifyTime) {
        this.modifyTime = modifyTime;
    }
}